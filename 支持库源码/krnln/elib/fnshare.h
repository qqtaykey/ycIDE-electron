...
#include "lib2.h"
typedef INT_PTR(cdecl* PFN_ON_SYS_NOTIFY) (INT nMsg, DWORD_PTR dwParam1, DWORD_PTR dwParam2);
#ifndef _private
...
{
    NotifySys(NRS_MFREE, (DWORD_PTR)p, 0);
}



// 是否为调试版, 这个值是支持库加载时取一次
#define isDebugVer __LIB2_DEFFUNNAME(jy_GetVer)
bool isDebugVer(int* r); // 获取当前版本是否为编译版


// 与易语言ide通讯子程序
#define ProcessNotifyLib __LIB2_DEFFUNNAME(jy_ProcessNotifyLib)
INT_PTR WINAPI ProcessNotifyLib(INT nMsg, DWORD_PTR dwParam1, DWORD_PTR dwParam2);
...
{
    return (void*)((INT_PTR)pBase + index * sizeof(INT));
}
